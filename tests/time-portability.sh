...
expect_ok() {  # expect_ok <desc> <command...> — passes if the command succeeds
    local desc="$1"; shift
...
roundtrip=$(iso_to_epoch "$(epoch_to_iso "$now_e")")
chk "epoch -> iso -> epoch"   "$now_e" "$roundtrip"
...
expect_ok  "YYYY/MM/DD-HH:MM"        parse_time "2026/06/11-14:30"
expect_ok  "YYYY-MM-DD HH:MM"        parse_time "2026-06-11 14:30"
expect_ok  "bare HH:MM"              parse_time "14:30"
...
if [[ "$_DATE_IS_GNU" == "1" ]]; then
    expect_ok  "'2 hours ago'"       parse_time "2 hours ago"
    expect_ok  "'yesterday 14:00'"   parse_time "yesterday 14:00"
else
...
expect_ok  "iso_days_ago 0"          iso_days_ago 0
expect_ok  "iso_days_ago 7"          iso_days_ago 7
expect_ok  "iso_month_start"         iso_month_start
...
expect_ok  "parse_date_to_fmt today"       parse_date_to_fmt "today"      "%Y-%m-%d"
