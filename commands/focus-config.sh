...
function focus_config_show() {
    ...
    echo
    echo "Database: $DB"
    echo "Data Directory: ${REFOCUS_STATE_DIR:-$HOME/.local/refocus}"
    echo "Log Directory: ${REFOCUS_STATE_DIR:-$HOME/.local/refocus}"
    echo "Error Log: ${REFOCUS_STATE_DIR:-$HOME/.local/refocus}/error.log"
    echo
    echo "Table Names:"
...
function focus_config_validate() {
    ...
    if [[ -z "${REFOCUS_DB_PATH:-$HOME/.local/refocus/refocus.db}" ]]; then
